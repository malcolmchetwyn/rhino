...
async def optimized_search_index(description, query):
    ...
    try:
        ...
        db = loaded_sf_user_guides_indexes.get(description)
        if db is None:
...
async def search_all_indices(query):
    ...
    for desc in descriptions:
        if desc not in loaded_sf_user_guides_indexes:
...
async def get_sf_user_guides_details(essential_query):
...
def preload_sf_user_guides_indices():
    #print("Pre-loading Salesforce User Guides indices...")
    logging.info("Pre-loading Salesforce User Guides indices...")
    for desc in descriptions:
        load_index(desc, embeddings)  # Adjust 'embeddings' if it's not globally available or needs to change per description
    print("All Salesforce User Guides indices pre-loaded.")
    logging.info("All Salesforce User Guides indices pre-loaded.")